...
internal/
  ...
  platform/
    ...
    app/             应用管理域（M2，小三层，docs/低代码平台/应用管理/，
                      ...
                      菜单个人收藏（ADR-011，docs/低代码平台/应用管理/
                      菜单收藏后端功能开发详情.md，P1/P2 已落地）：POST/
                      DELETE/GET /menu-favorites 幂等增删与跨应用「我的收藏」
                      游标列表（迁移 000081 补授 list），收藏资格由服务层
                      canFavorite 统一策略裁决（仅资产叶子节点 ∧ 应用可用 ∧
                      节点在成员有效可见集内，与菜单读取同源），列表读侧
                      复用同口径可见性裁剪（失效记录只过滤不删除），指标见
                      internal/metrics/menu.go（不入审计）
