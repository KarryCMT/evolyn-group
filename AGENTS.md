...
- `docs/` 下的 `docs.go`、`swagger.*`、`rbac.png` 为后端 swagger 生成产物，不要手工编辑；业务设计文档按子目录组织并配 `README.md` 导航，不要散落根目录。
- `docs/低代码平台/`：平台向低代码转型（对标简道云）的架构设计专题，设计基线与 ADR 见该目录下的《企业级低代码平台技术架构设计.md》，演进路线与目录映射见其 `README.md`。
