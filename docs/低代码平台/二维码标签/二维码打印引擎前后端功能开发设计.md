...
本轮已落地“标签模板生命周期”纵切，实施事实源为 OpenSpec change
`add-label-template-lifecycle`。当前范围不是本文所规划的完整 Print Engine，已完成边界如下：
...
后续 change 再实现批量任务、对象存储、短 Token、扫码落地页、表达式
执行器和打印机语言。本轮明确不为这些能力预建表或伪接口。PostgreSQL 生命周期、
并发发布、迁移 down/up 与安全边界已有真库测试；执行环境需配置 `TEST_PG_DSN`。
