...
- [表单运行时固定操作区开发实施方案](../表单设计器/表单运行时固定操作区开发实施方案.md)：以 `FormRuntimeSurface` 二次封装最终渲染器，统一预览抽屉、独立预览、应用工作区和移动 Web 的滚动内容与固定操作区。
...
| `@evolyn.do/form/runtime` | `src/runtime/` | 最终渲染器：会话 Store（`store/createFormRuntime.ts`）、按 `widget.type` 注册的字段注册表与基础 9 类组件（`widgets/`）、无具体按钮的执行内核（`renderer/FormRenderer.vue`）、固定操作区组合入口（`surface/FormRuntimeSurface.vue` + `FormRuntimeActionBar.vue`）、Adapter 契约（`adapters/types.ts`）、独立样式（`runtime/style.css`）；提交载荷 `{formId, publishedVersion, schemaRevision, values}`（值按 `widgetName` 取键） |
...
- Web 宿主接入：`apps/web/src/pages/form/preview.vue`（已发布走 bootstrap + 真实提交；未发布回退 sessionStorage 草稿本地回放）、`apps/web/src/components/form/FormDesignPreviewDrawer.vue`（桌面/移动设计预览）、`apps/web/src/components/application/runtime/ApplicationWorkspaceFormRuntime.vue`（应用工作区运行时 bootstrap + 真实提交）；三者统一使用 `FormRuntimeSurface`，主题在 Surface 边界映射到 `--el-*`。
