...
describe('Physical Engine', () => {
  ...
  it('marks removed columns as deprecated instead of producing a destructive operation', () => {
    ...
    const next = createPhysicalModel('app_order', [
      { id: 'field_amount', type: 'decimal', required: true },
    ]);

    expect(planPhysicalMigration(previous, next).operations).toEqual([
      expect.objectContaining({
        type: 'alterColumn',
        after: expect.objectContaining({ nullable: false }),
      }),
      expect.objectContaining({
        type: 'deprecateColumn',
        column: expect.objectContaining({ column: 'f_status' }),
      }),
    ]);
  });
});
