...
defineOptions({ name: 'BlankAppDialog' });

export interface BlankAppDraft {
  name: string;
  icon: BlankAppIcon;
}

export type BlankAppIcon = EvolynIconPickerValue;

const props = defineProps<{
  /** 异步提交处理：resolve true 表示创建成功（父级随后关闭弹窗）；false 表示失败，保持弹窗开启以保留填写内容 */
  submit: (draft: BlankAppDraft) => Promise<boolean>;
}>();

const emit = defineEmits<{
  /** 提交成功：由父级负责关闭弹窗（含上级「新建应用」弹窗） */
  success: [draft: BlankAppDraft];
}>();

const visible = defineModel<boolean>({ default: false });
const formRef = shallowRef<FormInstance>();
const submitting = shallowRef(false);
const form = reactive<BlankAppDraft>({
  name: '',
...
const rules: FormRules<BlankAppDraft> = {
  name: [
...
<template>
  ...
  >
    <template #header>
      <header class="blank-app-dialog__header">
        <h2 class="blank-app-dialog__heading">创建空白应用</h2>
        <button
          class="blank-app-dialog__close"
          type="button"
          aria-label="关闭创建空白应用"
          :disabled="submitting"
          @click="visible = false"
    ...
    >
      ...
      <el-form-item label="图标" prop="icon">
        <AppIconPicker v-model="iconValue" />
      </el-form-item>
    </el-form>

    <template #footer>
      <footer class="blank-app-dialog__footer">
...
.blank-app-dialog .el-dialog__header,
.blank-app-dialog .el-dialog__footer {
  flex: 0 0 auto;
  padding: 0;
  margin: 0;
}
...
.blank-app-dialog .el-dialog__body {
  flex: 1;
  min-height: 0;
...
.blank-app-dialog__form .el-form-item {
  margin-bottom: var(--el-space-3xl);
}

.blank-app-dialog__form .el-form-item__label {
  height: auto;
...
.blank-app-dialog__form .el-input__wrapper {
  min-height: 44px;
...
.blank-app-dialog__form .el-input__wrapper.is-focus {
  box-shadow: 0 0 0 1px var(--el-color-primary) inset;
}

.blank-app-dialog__form .el-input__inner {
  font-size: var(--el-font-size-base);
}

.blank-app-dialog__icon-list {
  display: flex;
  flex-wrap: wrap;
  gap: var(--el-space-md);
}

.blank-app-dialog__icon-option {
  display: inline-flex;
  width: 56px;
  height: 56px;
  padding: 0;
...
.blank-app-dialog__footer .el-button {
  min-width: 78px;
  height: 36px;
  margin: 0;
  font-size: var(--el-font-size-base);
