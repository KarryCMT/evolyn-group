...
import { appPersonalNavigation } from './appWorkspacePreview';

defineOptions({ name: 'AppWorkspaceSidebar' });
...
function flattenLeaves(assets: AppWorkspaceAsset[]): AppWorkspaceAsset[] {
...
function handleCreateAsset(command: string | number | object) {
  ...
  emit('createAsset', { type: command as AppWorkspaceCreateAssetType });
}
</script>

<template>
  <aside
    class="app-workspace-sidebar"
    :class="{
      'app-workspace-sidebar--collapsed': props.collapsed,
    }"
    aria-label="应用导航"
  >
    <div class="app-workspace-sidebar__content" :aria-hidden="props.collapsed">
      <header class="app-workspace-sidebar__header">
        <button
          class="app-workspace-sidebar__back"
          type="button"
          aria-label="返回工作台"
          @click="emit('back')"
        >
      ...
      <WorkflowCenterNavigation
        ...
        class="app-workspace-sidebar__workflow-nav"
      ...
      <nav v-else class="app-workspace-sidebar__personal-nav" aria-label="个人应用入口">
        <button
          v-for="item in appPersonalNavigation"
          :key="item.code"
          class="app-workspace-sidebar__nav-item"
          :class="{
            'app-workspace-sidebar__nav-item--active': props.activePersonalCode === item.code,
          }"
          type="button"
      ...
      <div class="app-workspace-sidebar__asset-tools">
        <label class="app-workspace-sidebar__search">
        ...
        <el-dropdown
          ...
          trigger="click"
          popper-class="app-workspace-root-create-actions"
          @command="handleCreateAsset"
        >
          <button class="app-workspace-sidebar__create" type="button" aria-label="新建应用资产">
            <RiAddFill aria-hidden="true" />
          </button>
          <template #dropdown>
            <el-dropdown-menu class="app-workspace-root-create-actions__menu">
              ...
              <el-dropdown-item
                class="app-workspace-root-create-actions__guide"
                divided
      ...
      <el-scrollbar class="app-workspace-sidebar__asset-scrollbar">
        <nav class="app-workspace-sidebar__asset-nav" aria-label="应用资产">
          <p v-if="props.menuStatus === 'loading'" class="app-workspace-sidebar__menu-tip">
            菜单加载中…
          </p>
          <p v-else-if="!hasAssets" class="app-workspace-sidebar__menu-tip">
            暂无应用资产，点击 + 创建第一个资产
          </p>
          <template v-else>
            <AppWorkspaceAssetItem
      ...
      <footer class="app-workspace-sidebar__footer">
        <button
          class="app-workspace-sidebar__management"
          type="button"
          aria-label="应用后台"
...
.app-workspace-sidebar {
  ...
  --app-workspace-menu-item-height: 42px;
  ...
  &__nav-item,
  &__management {
    min-height: var(--app-workspace-menu-item-height);
  ...
  &--collapsed {
    ...
    .app-workspace-sidebar__content {
      pointer-events: none;
      opacity: 0;
      visibility: hidden;
...
<style lang="scss">
.app-workspace-root-create-actions.el-popper {
  min-width: 232px;
  border-color: var(--el-border-color-lighter);
  box-shadow: var(--el-box-shadow-light);
}

.app-workspace-root-create-actions__menu.el-dropdown-menu {
...
.app-workspace-root-create-actions__menu .el-dropdown-menu__item {
  height: 42px;
  gap: var(--el-space-md);
...
.app-workspace-root-create-actions__guide.el-dropdown-menu__item {
