<script setup lang="ts">
import { RiFileList3Fill, RiLayoutGridFill } from '@remixicon/vue';
import type { AppWorkspaceAsset, AppWorkspaceMode } from './appWorkspace.types';

defineOptions({ name: 'AppContentPlaceholder' });

const props = defineProps<{
  /** 当前选中资产；菜单为空（M2-菜单-1 常态）时为 null，渲染空态占位 */
  asset: AppWorkspaceAsset | null;
  mode: AppWorkspaceMode;
}>();

const modeLabels: Record<AppWorkspaceMode, string> = {
  fill: '数据填写区',
  design: '设计区',
  data: '数据管理区',
};
</script>

<template>
  <main
    class="app-content-placeholder"
    :aria-label="props.asset ? `${props.asset.label}${modeLabels[props.mode]}` : '应用内容区'"
  >
    <div class="app-content-placeholder__content">
      <template v-if="props.asset">
        <component
          :is="props.asset.type === 'dashboard' ? RiLayoutGridFill : RiFileList3Fill"
          class="app-content-placeholder__icon"
          aria-hidden="true"
        />
        <h1 class="app-content-placeholder__title">{{ props.asset.label }}</h1>
        <p class="app-content-placeholder__description">
          {{ modeLabels[props.mode] }}已预留；表单渲染与设计能力将由后续独立包接入。
        </p>
      </template>
      <template v-else>
        <RiLayoutGridFill class="app-content-placeholder__icon" aria-hidden="true" />
        <h1 class="app-content-placeholder__title">暂无应用资产</h1>
        <p class="app-content-placeholder__description">
          通过左侧「新建应用资产」创建第一个表单或仪表盘后，将在此处打开。
        </p>
      </template>
    </div>
  </main>
</template>

<style scoped lang="scss">
.app-content-placeholder {
  display: grid;
  min-width: 0;
  min-height: 0;
  flex: 1;
  place-items: center;
  background: var(--el-bg-color);

  &__content {
    display: flex;
    max-width: 360px;
    padding: var(--el-space-4xl);
    align-items: center;
    flex-direction: column;
    color: var(--el-text-color-secondary);
    text-align: center;
  }

  &__icon {
    margin-bottom: var(--el-space-xl);
    color: var(--el-color-primary-light-5);
    font-size: 56px;
  }

  &__title {
    margin: 0 0 var(--el-space-md);
    color: var(--el-text-color-primary);
    font-size: var(--el-font-size-medium);
    font-weight: 650;
    line-height: 28px;
  }

  &__description {
    margin: 0;
    font-size: var(--el-font-size-base);
    line-height: 22px;
  }
}
</style>
