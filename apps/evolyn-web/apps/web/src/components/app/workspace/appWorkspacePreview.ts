...
export type AppPersonalNavigationCode = 'todo' | 'started' | 'handled' | 'copied' | 'dashboard';

/**
 * 应用工作区侧栏的个人导航入口（我的待办/我发起的等）：不属于应用资产
 * 树（应用菜单接口不返回，见应用菜单接口功能设计方案 §2「homeList 不随
 * 菜单返回」），属于个人首页域，后续接入真实待办数据时替换。资产区
 * 数据源已切换为应用菜单接口（useAppMenu）。
 */
export const appPersonalNavigation = [
  ...
  code: AppPersonalNavigationCode;
  label: string;
