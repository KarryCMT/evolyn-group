...
import CrossAppAppList from './CrossAppAppList.vue';
...
const activeAppFormCount = computed(
  () => activeApp.value?.groups.reduce((count, group) => count + group.forms.length, 0) ?? 0,
);
  ...
  <section class="cross-app-setting">
    ...
    <div class="cross-app-setting__body">
      ...
      <div class="cross-app-setting__picker">
        <aside class="cross-app-setting__apps" aria-label="可调用应用">
          <CrossAppAppList
            :apps="filteredApps"
            :active-app-id="activeApp?.id ?? activeAppId"
            :selected-form-ids="selectedFormIds"
            @select="selectApp"
          />
        </aside>
        <section class="cross-app-setting__tree" aria-label="应用表单列表">
          <header v-if="activeApp" class="cross-app-setting__tree-heading">
            <strong>{{ activeApp.name }}</strong>
            <span>{{ activeAppFormCount }} 张表单</span>
          </header>
          <CrossAppFormTree
            v-if="activeApp"
