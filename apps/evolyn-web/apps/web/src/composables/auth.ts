...
// sessionActive/userInfo 以 readonly 暴露，变更一律走 store 提供的动作（login/logout/
// switchTenant 等），消费方（路由守卫/登录页/注册页）签名与 store 化之前保持一致
export function useAuth() {
  const store = useAuthStore();
  // storeToRefs 保持响应式：state/getter 提取为 ref，action 本身已绑定 store 可直接取
  const { sessionActive, userInfo, isAuthenticated, displayName, isTenantOwner } = storeToRefs(store);
