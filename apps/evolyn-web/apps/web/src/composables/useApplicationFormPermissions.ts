 ...
 */
export function useApplicationFormPermissions(appCode: Readonly<Ref<string>>) {
...
function formAssetsFromMenu(
  menu: Awaited<ReturnType<typeof getApplicationMenuByCode>>,
): PermissionAsset[] {
  const childrenByParent = new Map<string | null, (typeof menu.entryMap)[string][]>();
  for (const entry of Object.values(menu.entryMap)) {
  ...
  const build = (parentID: string | null): PermissionAsset[] =>
    (childrenByParent.get(parentID) ?? [])
      .sort(
        (left, right) =>
          left.sortOrder - right.sortOrder || left.entryId.localeCompare(right.entryId),
      )
      .flatMap((entry): PermissionAsset[] => {
        if (entry.type === 'group') {
          const children = build(entry.entryId);
          return children.length
            ? [{ id: entry.entryId, name: entry.name, type: 'group', children }]
            : [];
        }
        if (entry.type !== 'form' || entry.target?.type !== 'form') return [];
        return [
          {
            id: entry.entryId,
            name: entry.name,
            type: entry.target.formType === 'workflow' ? 'workflow-form' : 'form',
            formCode: entry.target.code,
          },
        ];
      });
  return build(null);
