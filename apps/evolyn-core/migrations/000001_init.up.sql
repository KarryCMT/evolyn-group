-- 初始基线：整改前（weave 演进）既有 Schema 与种子数据快照。
-- 后续 000002+ 迁移在此基础上做租户化/约束修复；空库执行全链即达终态。
-- 注意：与 scripts/db.sql（终态冷启动快照）不同，本文件保留历史原貌。

-- 租户（平台一级资源；历史版本 tenants 表自身带 tenant_id 列，000004 移除）
