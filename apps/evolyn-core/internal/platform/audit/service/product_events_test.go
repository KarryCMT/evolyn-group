...
func TestKnownProductCodes(t *testing.T) {
	...
	assert.True(t, KnownProductCategory("app")) // 应用管理归产品日志
...
func TestRecordCarriesAppSnapshot(t *testing.T) {
	// 应用维度快照（000064）：Entry 填写应用三元组时写时固化；AppID
	...
	log := repo.created[0]
	require.NotNil(t, log.AppID)
	assert.Equal(t, uint(21), *log.AppID)
	assert.Equal(t, "app_21", log.AppCode)
	assert.Equal(t, "测试应用", log.AppNameSnapshot)
	// 产品分类与资源动词覆盖同步生效
	...
	assert.Nil(t, repo2.created[0].AppID)
	assert.Empty(t, repo2.created[0].AppCode)
	assert.Empty(t, repo2.created[0].AppNameSnapshot)
}
