...
type AppRepository interface {
	...
	GetByCode(ctx context.Context, code string) (*model.App, error)
	// List 游标分页查询；返回当页数据与是否还有下一页（limit+1 探测）
	List(ctx context.Context, params ListParams) ([]model.App, bool, error)
	...
	SoftDelete(ctx context.Context, app *model.App) error
...
// MenuSnapshot 应用与菜单节点的同快照读取结果：menuRevision 与 nodes
// 来自单条 SQL 的同一语句快照（方案 §5.2：禁止 Read Committed 两读拼接，
// 否则客户端可能取得「旧树 + 新修订号」在全量重排时漏检冲突）
type MenuSnapshot struct {
	AppID   uint
	AppCode string
	// AppName 应用名称（000064：菜单写路径审计的应用维度快照源）
	AppName         string
	Status          string
...
type MenuRepository interface {
	...
	UpdateAppearanceByFormTarget(ctx context.Context, appID, formID uint, icon, color string) error
	// SoftDeleteByFormTarget 软删表单目标节点（表单删除事务内）
	SoftDeleteByFormTarget(ctx context.Context, appID, formID uint) error
	// MaxSortOrder 指定父节点（nil 即根级）下最大排序值；无节点返回 0
	MaxSortOrder(ctx context.Context, appID uint, parentMenuID *uint) (int64, error)
	// FindByCode 按节点编码查未软删节点（表单挂载父分组定位用）
	FindByCode(ctx context.Context, appID uint, code string) (*model.MenuNode, error)
	// BumpMenuRevision 同事务递增应用菜单修订号（菜单写入的并发口令）
	BumpMenuRevision(ctx context.Context, appID uint) error
