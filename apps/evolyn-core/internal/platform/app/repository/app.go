...
func (r *appRepository) withContext(ctx context.Context) *gorm.DB {
	return infrastructure.ResolveDB(ctx, r.db)
}

func (r *appRepository) Create(ctx context.Context, app *model.App) (*model.App, error) {
...
func (r *appRepository) CreateInstallation(ctx context.Context, inst *model.Installation) error {
...
func (r *appRepository) GetByCode(ctx context.Context, code string) (*model.App, error) {
	app := new(model.App)
...
func (r *appRepository) List(ctx context.Context, params ListParams) ([]model.App, bool, error) {
	query := r.withContext(ctx).Model(&model.App{})
	if params.Keyword != "" {
...
func (r *appRepository) UpdateFields(ctx context.Context, id uint, fields map[string]interface{}) error {
	return r.withContext(ctx).Model(&model.App{}).Where("id = ?", id).Updates(fields).Error
}

func (r *appRepository) SoftDelete(ctx context.Context, app *model.App) error {
	return r.withContext(ctx).Delete(app).Error
}

// CountBillableByTenant 显式 Scope 而非依赖请求租户上下文（配额执行路径
// 可能在无上下文的事务内）；GORM 软删行自动排除
func (r *appRepository) CountBillableByTenant(ctx context.Context, tenantID uint) (int64, error) {
	var count int64
	err := r.withContext(ctx).Model(&model.App{}).
		Scopes(infrastructure.TenantScope(tenantID)).Count(&count).Error
	return count, err
}

// Migrate 开发/测试路径：AutoMigrate 建表 + 补齐 GORM 标签表达不了的
// 软删部分唯一索引（与迁移链 uk_tn_apps_tenant_code 同名同构）
func (r *appRepository) Migrate() error {
	if err := r.db.AutoMigrate(&model.App{}, &model.Installation{}); err != nil {
		return err
	}
	return r.db.Exec(`CREATE UNIQUE INDEX IF NOT EXISTS uk_tn_apps_tenant_code
		ON apps (tenant_id, code) WHERE deleted_at IS NULL`).Error
}
