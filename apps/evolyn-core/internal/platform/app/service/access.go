...
// AppAccessEvaluator 应用级访问判定（§9.1）：capabilities 计算与
...
type AppAccessEvaluator interface {
...
// 种子给全体成员 apps:view 等），口径对齐 AuthorizationMiddleware
type rbacAccessEvaluator struct {
	members memberLoader
	groups  groupRoleLoader
}

func NewRBACAccessEvaluator(members memberLoader, groups groupRoleLoader) AppAccessEvaluator {
...
func (e *rbacAccessEvaluator) Permissions(ctx context.Context, member *iammodel.User) map[string]bool {
	...
	// 调用方构造的「TenantID=目标租户 + apps:* 角色」不会生效——
	// 重载只认数据库里的真实角色；ID 不属于本租户时重载即 NotFound
	reloaded, err := e.members.GetUserByID(ctx, member.ID)
	if err != nil {
		if err != gorm.ErrRecordNotFound {
			logrus.Warnf("app access evaluator: load member %d failed: %v", member.ID, err)
		}
		return map[string]bool{}
	}
	...
	if err != nil {
		logrus.Warnf("app access evaluator: load group %s failed: %v", iammodel.AuthenticatedGroup, err)
		return perms
	}
	if group != nil {
