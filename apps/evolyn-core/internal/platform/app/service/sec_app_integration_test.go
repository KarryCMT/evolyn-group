...
//
// 验证链路覆盖：Tenant Context → AppService（CheckAndReserve 事务）
// → AppRepository → GORM Callback → PostgreSQL 行锁/部分唯一索引。
...
func newAppEnv(t *testing.T) *appEnv {
	...
	env := &appEnv{
		...
		iamRepo:    iamRepo,
		appRepo:    appRepo,
		appSvc:     NewAppService(txManager, appRepo, quotaSvc, auditSvc, NewRBACAccessEvaluator(iamRepo.User(), iamRepo.Group())),
		quotaSvc:   quotaSvc,
...
func TestSECAPP001CrossTenantAccess(t *testing.T) {
	...
	_, err = env.appSvc.Update(appCtx(env.alpha.ID), env.alphaMember, created.ID, &model.UpdateAppRequest{})
	assert.True(t, errors.Is(err, apperrors.ErrNotFound))
	...
	// beta 行仍在库（软删未发生）
	assert.EqualValues(t, 1, env.rawCount(t, "SELECT COUNT(*) FROM tn_apps WHERE id = ?", created.ID))
}
...
func TestSECAPP002CrossTenantMemberBinding(t *testing.T) {
	...
	assert.Zero(t, env.rawCount(t, "SELECT COUNT(*) FROM tn_apps WHERE tenant_id = ?", env.alpha.ID))
	assert.Zero(t, env.rawCount(t, "SELECT COUNT(*) FROM tn_app_installations WHERE tenant_id = ?", env.alpha.ID))
}
...
func TestSECAPP003ListTenantScoped(t *testing.T) {
	...
	alphaPage, err := env.appSvc.List(appCtx(env.alpha.ID), env.alphaMember, model.ListAppsQuery{})
	assert.NoError(t, err)
...
func TestSECAPP005CrossTenantRoleLeak(t *testing.T) {
	...
	_, err = env.appSvc.List(appCtx(env.alpha.ID), env.betaMember, model.ListAppsQuery{})
	...
	selfPage, err := env.appSvc.List(appCtx(env.beta.ID), env.betaMember, model.ListAppsQuery{})
	assert.NoError(t, err)
	assert.Empty(t, selfPage.Items)
}

// SEC-APP-006：伪造/陈旧成员对象不生效——evaluator 按 member.ID 在当前
// 租户重载真实角色，调用方在 User 对象上伪造的 TenantID/apps:*
// 角色不影响判定；凭空 ID 一律空集
func TestSECAPP006ForgedMemberObject(t *testing.T) {
	...
	t.Run("真实 ID + 伪造通配角色：写路径仍按真实角色拒绝", func(t *testing.T) {
		...
		_, err = env.appSvc.Update(appCtx(env.alpha.ID), forged, created.ID, &model.UpdateAppRequest{})
	...
	t.Run("凭空 ID + 匹配租户 + 通配角色：权限集为空", func(t *testing.T) {
		...
		_, err = env.appSvc.List(appCtx(env.alpha.ID), ghost, model.ListAppsQuery{})
		assert.True(t, errors.Is(err, apperrors.ErrForbidden))
	})
...
func TestQUOTAAPP001LimitEnforced(t *testing.T) {
	...
	assert.EqualValues(t, 2, env.rawCount(t, "SELECT COUNT(*) FROM tn_apps WHERE tenant_id = ?", env.alpha.ID))
	assert.EqualValues(t, 2, env.rawCount(t, "SELECT COUNT(*) FROM tn_app_installations WHERE tenant_id = ?", env.alpha.ID))
}
...
func TestQUOTAAPP002ConcurrentCreate(t *testing.T) {
	...
	assert.EqualValues(t, limit, env.rawCount(t, "SELECT COUNT(*) FROM tn_apps WHERE tenant_id = ?", env.alpha.ID))
}

// SEC-APP-004：普通成员（仅 authenticated 系统组基线，无显式角色）的
// capabilities 与鉴权中间件同源——AccessEvaluator 合并系统组规则后
// view=true（基线 apps:view）、edit/delete=false
func TestSECAPP004PlainMemberCapabilities(t *testing.T) {
	...
	assert.NoError(t, err)
	assert.True(t, detail.Capabilities.View, "authenticated 基线 apps:view 应并入权限集")
	assert.False(t, detail.Capabilities.Edit)
	assert.False(t, detail.Capabilities.Delete)
...
func TestQUOTAAPP003SoftDeleteReleasesQuota(t *testing.T) {
	...
	assert.NoError(t, err)
	assert.EqualValues(t, 1, env.rawCount(t, "SELECT COUNT(*) FROM tn_apps WHERE tenant_id = ? AND deleted_at IS NULL", env.alpha.ID))
	assert.EqualValues(t, 2, env.rawCount(t, "SELECT COUNT(*) FROM tn_app_installations WHERE tenant_id = ?", env.alpha.ID))
	_ = second
}
