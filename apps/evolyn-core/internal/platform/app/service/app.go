...
// 演进决策，前端只回传键值不自造
var (
	appColors = set.NewString("primary")
)
...
// appService 应用服务实现。配额执行来自租户域（依赖方向
// app→tenant/service 单向），审计提交后 best-effort 写入；
// 访问判定统一经 AppAccessEvaluator（§9.1），capabilities 与
// 写路径复核不依赖 HTTP 中间件
...
func (s *appService) CreateBlank(ctx context.Context, member *iammodel.User, req *model.CreateBlankRequest) (*model.AppDetail, error) {
	...
	// 与路由层 POST /apps（verb=create）同口径的 Service 复核：
	// 内部调用路径同样拦截无 apps:create 的成员（未认证/跨租户
	// 成员的权限集为空，见 rbacAccessEvaluator 归属守卫）
	if !s.access.Permissions(ctx, member)["apps:create"] {
		return nil, httpx.Wrap(apperrors.ErrForbidden,
			fmt.Errorf("member %d cannot create app in tenant %d", member.ID, tenantID))
	}
	...
	if s.audit != nil {
		s.audit.Record(ctx, auditservice.Entry{
			Module: "app", Action: "create", ResourceType: "app",
...
func (s *appService) provisionBlank(
	ctx context.Context, tenantID uint, member *iammodel.User, name string, icon model.AppIcon, color string,
) (*model.App, *model.Installation, error) {
	var (
		app  *model.App
		inst *model.Installation
	)
	err := s.quota.CheckAndReserve(ctx, tenantID, tenantmodel.QuotaApps, func(tctx context.Context) error {
		code, cerr := newAppCode()
		if cerr != nil {
			return cerr
		}
		...
		draft := &model.App{
			...
			// 与菜单变更同一事务切换为 app，不能由当前成员权限反推。
			HomeMode: model.AppHomeModeBuilder,
		}
...
// newAppCode 生成应用编码：app_ + 16 位随机 hex。租户内唯一由
// uk_tn_apps_tenant_code 部分唯一索引兜底，随机空间下冲突可忽略
func newAppCode() (string, error) {
	buf := make([]byte, 8)
...
func (s *appService) List(ctx context.Context, member *iammodel.User, query model.ListAppsQuery) (*model.AppPage, error) {
	// 与路由层 GET /apps（verb=list）同口径的 Service 复核；
	// 权限集同时用于逐条 capabilities 派生，请求内只计算一次
	perms := s.access.Permissions(ctx, member)
	if !perms["apps:list"] {
		return nil, httpx.Wrap(apperrors.ErrForbidden,
			fmt.Errorf("member %d cannot list apps", memberID(member)))
	}
	...
	if query.Status != "" &&
		query.Status != model.AppStatusActive && query.Status != model.AppStatusArchived {
	...
	page := &model.AppPage{Items: make([]model.AppDetail, 0, len(apps)), HasMore: hasMore}
	for i := range apps {
...
func (s *appService) Get(ctx context.Context, member *iammodel.User, id uint) (*model.AppDetail, error) {
	app, err := s.load(ctx, id)
	if err != nil {
		return nil, err
	}
	// 与路由层 GET /apps/:id（verb=get）同口径的 Service 复核，
	// 内部调用路径不依赖中间件也能拦住越权读取
	perms := s.access.Permissions(ctx, member)
	if !perms["apps:get"] {
		return nil, httpx.Wrap(apperrors.ErrForbidden,
			fmt.Errorf("member %d cannot get app %d", memberID(member), app.ID))
	}
...
func (s *appService) GetByCode(ctx context.Context, member *iammodel.User, code string) (*model.AppDetail, error) {
	...
	if !perms["apps:get"] {
		return nil, httpx.Wrap(apperrors.ErrForbidden,
			fmt.Errorf("member %d cannot get app %s", memberID(member), code))
	}
	return s.detailFor(perms, app), nil
}
...
func (s *appService) Update(ctx context.Context, member *iammodel.User, id uint, req *model.UpdateAppRequest) (*model.AppDetail, error) {
	...
	if !perms["apps:patch"] {
		return nil, httpx.Wrap(apperrors.ErrForbidden,
			fmt.Errorf("member %d cannot update app %d", memberID(member), id))
	}
	...
	if isProvisioning(app) {
		return nil, httpx.Wrap(apperrors.ErrProvisioning,
			fmt.Errorf("app %d provisioning (%s)", app.ID, app.ProvisionStatus))
	}
	...
	if s.audit != nil {
		s.audit.Record(ctx, auditservice.Entry{
			Module: "app", Action: action, ResourceType: "app",
			ResourceID: strconv.FormatUint(uint64(app.ID), 10),
			Before:     before,
			After:      fields,
...
func (s *appService) Delete(ctx context.Context, member *iammodel.User, id uint) error {
	// 与路由层 DELETE（verb=delete）同口径的 Service 复核
	if perms := s.access.Permissions(ctx, member); !perms["apps:delete"] {
		return httpx.Wrap(apperrors.ErrForbidden,
			fmt.Errorf("member %d cannot delete app %d", memberID(member), id))
	}
	...
	if s.audit != nil {
		s.audit.Record(ctx, auditservice.Entry{
			Module: "app", Action: "delete", ResourceType: "app",
			ResourceID: strconv.FormatUint(uint64(app.ID), 10),
			After:      map[string]any{"name": app.Name, "source": app.SourceType},
...
// 扩展实现）落地后再叠加
func (s *appService) detailFor(perms map[string]bool, app *model.App) *model.AppDetail {
	// 编辑前提：active 且实例化已收敛（ready/failed 可改，失败应用可修后再试）
	editableState := app.Status == model.AppStatusActive && !isProvisioning(app)
	deletableState := !isProvisioning(app)

	return &model.AppDetail{
		ID:              app.ID,
		Code:            app.Code,
		Name:            app.Name,
		Icon:            app.Icon,
		Color:           app.Color,
		Source:          model.AppSource{Type: app.SourceType, Channel: channelForSourceType(app.SourceType)},
		Status:          app.Status,
		...
		Capabilities: model.AppCapabilities{
			View: perms["apps:get"] || perms["apps:list"],
			// edit 只认 patch（与路由 PATCH 动词一致，本域无 PUT 路由）
			Edit:   perms["apps:patch"] && editableState,
			Delete: perms["apps:delete"] && deletableState,
		},
	}
}
...
func (s *appService) load(ctx context.Context, id uint) (*model.App, error) {
...
func (s *appService) loadByCode(ctx context.Context, code string) (*model.App, error) {
...
func isProvisioning(app *model.App) bool {
...
func normalizeAppearance(name string, icon *model.AppIcon, color string) (string, model.AppIcon, string, error) {
	name = strings.TrimSpace(name)
	if name == "" || utf8.RuneCountInString(name) > maxNameRunes {
		return "", model.AppIcon{}, "", httpx.Wrap(apperrors.ErrNameInvalid, fmt.Errorf("invalid name length"))
	}
	storedIcon, err := normalizeIcon(icon)
	if err != nil {
		return "", model.AppIcon{}, "", err
	}
	if color == "" {
		color = defaultColor
	}
	if !appColors.Has(color) {
		return "", model.AppIcon{}, "", httpx.Wrap(apperrors.ErrColorInvalid, fmt.Errorf("unknown color %q", color))
	}
...
func normalizeIcon(icon *model.AppIcon) (model.AppIcon, error) {
	...
	case "remix":
		...
		if payload.Name == "" || len(payload.Name) > 64 || !strings.Contains(payload.Background, ",") {
			return model.AppIcon{}, httpx.Wrap(apperrors.ErrIconInvalid, fmt.Errorf("invalid remix icon"))
		}
	case "custom":
		if !strings.HasPrefix(payload.Name, "/api/v1/files/") || !strings.HasSuffix(payload.Name, "/content") {
			return model.AppIcon{}, httpx.Wrap(apperrors.ErrIconInvalid, fmt.Errorf("invalid custom icon"))
		}
	default:
		return model.AppIcon{}, httpx.Wrap(apperrors.ErrIconInvalid, fmt.Errorf("unknown icon type"))
	}
	encoded, err := json.Marshal(payload)
	if err != nil || len(encoded) > maxIconLength {
		return model.AppIcon{}, httpx.Wrap(apperrors.ErrIconInvalid, fmt.Errorf("icon too long"))
	}
	return payload, nil
}

func defaultIconPayload() model.AppIcon {
	return model.AppIcon{Type: "remix", Name: defaultIcon, Background: defaultIconBackground}
}
