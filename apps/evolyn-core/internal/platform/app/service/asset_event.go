...
// 注册表 app.asset.changed 的模板「{actorName}{verb}「{appName}」」配套）
const (
...
func (s *appService) UseAssetNotifier(notifier AssetEventNotifier) {
	s.notifier = notifier
}

// publishAssetChangedInTx 在业务事务内发布应用资产变更事件；事件编码为
// 「app:asset:{应用ID}:{动词}:{随机}」——业务事务重试（回滚后重放）
// 生成新事件 ID，同一业务提交只物化一条消息
func (s *appService) publishAssetChangedInTx(
	ctx context.Context, verb string, app *model.App, actor *iammodel.User,
) error {
	...
	return s.notifier.NotifyAssetChanged(ctx,
		fmt.Sprintf("app:asset:%d:%s", app.ID, hex.EncodeToString(suffix)),
