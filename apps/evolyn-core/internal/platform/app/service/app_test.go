...
func (f *fakeAppRepo) Create(ctx context.Context, app *model.App) (*model.App, error) {
	f.nextID++
	app.ID = f.nextID
	clone := *app
	f.apps[app.ID] = &clone
...
func (f *fakeAppRepo) CreateInstallation(ctx context.Context, inst *model.Installation) error {
	clone := *inst
	f.installs[inst.AppID] = &clone
	return nil
}

func (f *fakeAppRepo) GetByID(ctx context.Context, id uint) (*model.App, error) {
	if f.getByIDErr != nil {
...
func (f *fakeAppRepo) GetByCode(ctx context.Context, code string) (*model.App, error) {
...
func (f *fakeAppRepo) List(ctx context.Context, params repository.ListParams) ([]model.App, bool, error) {
...
func (f *fakeAppRepo) setField(app *model.App, key string, value interface{}) {
	switch key {
	case "name":
		app.Name = value.(string)
	case "icon":
		app.Icon = value.(model.AppIcon)
	case "color":
...
func (f *fakeAppRepo) SoftDelete(ctx context.Context, app *model.App) error {
...
// fullPerms 全量权限集（模拟 tenant-admin 的 apps:* 规则展开）
...
func TestRBACAccessEvaluatorGuard(t *testing.T) {
	...
	t.Run("伪造通配角色的快照被忽略（以重载角色为准）", func(t *testing.T) {
		// 调用方构造：真实成员 ID + 伪造 apps:* 角色
		forged := alphaMember(iammodel.Rule{Resource: "*", Operation: iammodel.AllOperation})
		perms := evaluator.Permissions(alphaCtx(), forged)
		assert.True(t, perms["apps:get"], "基线 view 经系统组合并")
		assert.True(t, perms["apps:list"])
		assert.False(t, perms["apps:patch"], "伪造的通配角色不得生效")
		assert.False(t, perms["apps:delete"])
	})
	...
	t.Run("系统组读取失败降级为重载后的显式角色集", func(t *testing.T) {
		...
		admin.Roles = []iammodel.Role{{Rules: iammodel.Rules{
			{Resource: "apps", Operation: iammodel.AllOperation},
		}}}
		...
		assert.True(t, perms["apps:patch"], "显式角色不受系统组降级影响")
		assert.True(t, perms["apps:get"])
	})
}

func newTestService(repo repository.AppRepository, quota fakeQuota, audit auditservice.Recorder, access AppAccessEvaluator) AppService {
	return NewAppService(passThroughTx{}, repo, quota, audit, access)
}
...
func TestCreateBlankValidation(t *testing.T) {
	...
	cases := []struct {
		...
		{"非法图标", model.CreateBlankRequest{Name: "应用", Icon: &model.AppIcon{Type: "remix", Name: "RiBookmarkFill"}}, apperrors.ErrIconInvalid},
...
func TestCreateBlankSuccess(t *testing.T) {
	...
	assert.Equal(t, model.AppIcon{Type: "remix", Name: "bookmark", Background: "#f7be54,#eda426"}, detail.Icon)
	...
	assert.Equal(t, model.AppStatusActive, detail.Status)
	assert.Equal(t, model.ProvisionStatusReady, detail.ProvisionStatus)
	assert.Equal(t, model.AppHomeModeBuilder, detail.HomeMode)
	...
	assert.Equal(t, "app", audit.entries[0].ResourceType)
...
func TestUpdateWhitelistAndTransitions(t *testing.T) {
	...
	// 白名单字段更新
	newName := "客户管理"
	updated, err := svc.Update(alphaCtx(), alphaMember(), created.ID, &model.UpdateAppRequest{Name: &newName})
	assert.NoError(t, err)
	assert.Equal(t, "客户管理", updated.Name)

	// 归档：status active→archived
	archived := model.AppStatusArchived
	updated, err = svc.Update(alphaCtx(), alphaMember(), created.ID, &model.UpdateAppRequest{Status: &archived})
	assert.NoError(t, err)
	assert.Equal(t, model.AppStatusArchived, updated.Status)
	// 归档态不可编辑（capabilities 派生）
	assert.False(t, updated.Capabilities.Edit)

	// 恢复：archived→active
	active := model.AppStatusActive
	updated, err = svc.Update(alphaCtx(), alphaMember(), created.ID, &model.UpdateAppRequest{Status: &active})
	assert.NoError(t, err)
	assert.Equal(t, model.AppStatusActive, updated.Status)

	// 非法状态值拒绝
	bad := "deleted"
	_, err = svc.Update(alphaCtx(), alphaMember(), created.ID, &model.UpdateAppRequest{Status: &bad})
	assert.True(t, errors.Is(err, apperrors.ErrStatusInvalid))

	// 空请求幂等返回
	same, err := svc.Update(alphaCtx(), alphaMember(), created.ID, &model.UpdateAppRequest{})
	assert.NoError(t, err)
...
func TestProvisioningGuard(t *testing.T) {
	...
	_, err = svc.Update(alphaCtx(), alphaMember(), created.ID, &model.UpdateAppRequest{})
...
func TestCapabilitiesByPermission(t *testing.T) {
	...
	viewOnly := map[string]bool{"apps:get": true, "apps:list": true}
	...
	cases := []struct {
		...
		{"全量权限 + active", fullPerms(), model.AppStatusActive, true, true, true},
		{"只读权限（authenticated 基线）", viewOnly, model.AppStatusActive, true, false, false},
		{"无权限", map[string]bool{}, model.AppStatusActive, false, false, false},
		{"全量权限 + 归档态（不可编辑）", fullPerms(), model.AppStatusArchived, true, false, true},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			app := &model.App{
				ID: 1, Status: tc.appStatus, ProvisionStatus: model.ProvisionStatusReady,
				HomeMode: model.AppHomeModeApp,
			}
			svc := newTestService(newFakeAppRepo(), fakeQuota{}, nil, fakeAccess{perms: tc.perms}).(*appService)
			...
			assert.Equal(t, model.AppHomeModeApp, detail.HomeMode)
		})
	}
}
...
var viewOnly = fakeAccess{perms: map[string]bool{"apps:get": true, "apps:list": true}}
...
func TestWritePathAccessGuards(t *testing.T) {
	...
	t.Run("List 无 list 权限拒绝", func(t *testing.T) {
		...
		noList := newTestService(repo, fakeQuota{}, nil, fakeAccess{perms: map[string]bool{"apps:get": true}})
		_, err = noList.List(alphaCtx(), alphaMember(), model.ListAppsQuery{})
		assert.True(t, errors.Is(err, apperrors.ErrForbidden))
	})
	...
	t.Run("仅 update 动词不放行编辑（路由只认 patch）", func(t *testing.T) {
		...
		updateOnly := newTestService(repo, fakeQuota{}, nil,
			fakeAccess{perms: map[string]bool{"apps:update": true, "apps:get": true}})
		_, err = updateOnly.Update(alphaCtx(), alphaMember(), created.ID, &model.UpdateAppRequest{})
		assert.True(t, errors.Is(err, apperrors.ErrForbidden))

		// capabilities.edit 同口径：仅 update 动词不产生编辑能力
		svc := updateOnly.(*appService)
		detail := svc.detailFor(map[string]bool{"apps:update": true}, repo.apps[created.ID])
	...
	t.Run("Update/Delete 无写权限拒绝且不落库", func(t *testing.T) {
		...
		readonly := newTestService(repo, fakeQuota{}, nil, viewOnly)
		_, err = readonly.Update(alphaCtx(), alphaMember(), created.ID, &model.UpdateAppRequest{})
		assert.True(t, errors.Is(err, apperrors.ErrForbidden))
