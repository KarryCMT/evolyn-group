...
// 查询路由可与 GET /apps/:id 并存注册
func TestRegisterRouteNoConflict(t *testing.T) {
	gin.SetMode(gin.TestMode)
	r := gin.New()
	// 仅验证路由树注册，handler 不会被调用，service 传 nil 安全
	NewAppController(nil).RegisterRoute(r.Group("/api/v1"))
	NewMenuController(nil).RegisterRoute(r.Group("/api/v1"))

	codeRouted := false
	createGroupRouted := false
	for _, route := range r.Routes() {
		if route.Path == "/api/v1/apps/code/:code" {
			codeRouted = true
		}
		if route.Method == "POST" && route.Path == "/api/v1/apps/code/:code/menu/groups" {
