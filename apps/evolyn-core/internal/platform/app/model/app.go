...
// 应用首页形态：builder 为首次构建引导，app 为运行时应用首页。
// 该状态由应用生命周期维护，不根据当前成员可见菜单数量推导，避免权限
// 过滤或菜单请求失败时将已有应用误判为空白应用。
const (
	AppHomeModeBuilder = "builder"
	AppHomeModeApp     = "app"
)
...
// App 应用实例（租户内一级资源）：所有业务读写经 GORM 租户
...
type App struct {
	...
	Icon              AppIcon `json:"icon" gorm:"type:jsonb;not null"`
	...
	HomeMode          string  `json:"homeMode" gorm:"size:16;not null;default:builder"` // builder / app
...
func (*App) TableName() string { return "tn_apps" }

// Installation 安装记录：应用创建来源快照（§6.5），一应用一条、追加写。
// 刻意不嵌 TenantBaseModel（无软删/更新语义）：读取一律经 app_id
// 关联（应用本身已租户过滤），不做独立租户路径查询
type Installation struct {
	ID                  uint            `json:"id" gorm:"autoIncrement;primaryKey"`
	TenantID            uint            `json:"tenantId" gorm:"index;not null;default:1"`
	AppID               uint            `json:"appId" gorm:"uniqueIndex;not null"`
...
func (*Installation) TableName() string { return "tn_app_installations" }
