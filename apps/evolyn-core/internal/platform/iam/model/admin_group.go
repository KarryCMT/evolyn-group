...
// 角色绑定实时推导、不落 admin_group_members 表；scope_config 仅自定义组生效
...
type AdminGroupMember struct {
	ID           uint `json:"id" gorm:"autoIncrement;primaryKey"`
	AdminGroupID uint `json:"adminGroupId" gorm:"not null;uniqueIndex:uk_admin_group_member,priority:1"`
	MemberID     uint `json:"memberId" gorm:"not null;uniqueIndex:uk_admin_group_member,priority:2"`
	// TenantID 显式声明：seed/回填路径可能无租户上下文，与 Callback 回填口径一致
	TenantID  uint      `json:"tenantId" gorm:"not null;index"`
	CreatedAt time.Time `json:"createdAt"`
}

func (*AdminGroupMember) TableName() string { return "admin_group_members" }
