...
func (f *fakeFormRepo) ExistingFormIDs(ctx context.Context, ids []uint) (map[uint]bool, error) {
	existing := map[uint]bool{}
	for _, id := range ids {
		if _, ok := f.forms[id]; ok {
			existing[id] = true
		}
	}
	return existing, nil
...
func TestCreateForm(t *testing.T) {
	...
	svc := newTestService(quota, newFakeFormRepo(), newFakeVersionRepo(), &fakeRecordRepo{}, nil)

	detail, err := svc.Create(tenantCtx(1), memberOfTenant(1), &model.CreateFormRequest{ApplicationID: 7, Name: "报名表"})
	...
	// 归档应用拒绝
	_, err = svc.Create(tenantCtx(1), memberOfTenant(1), &model.CreateFormRequest{ApplicationID: 8, Name: "x"})
	assert.ErrorIs(t, err, apperrors.ErrFormAppInvalid)

	// 配额超限
	quota.limit = 0
	_, err = svc.Create(tenantCtx(1), memberOfTenant(1), &model.CreateFormRequest{ApplicationID: 7, Name: "x"})
	assert.ErrorIs(t, err, tenantservice.ErrQuotaExceeded)

	// 跨租户成员拒绝
	_, err = svc.Create(tenantCtx(1), memberOfTenant(2), &model.CreateFormRequest{ApplicationID: 7, Name: "x"})
...
func TestCreateFormForwardsParentEntryCode(t *testing.T) {
	...
	_, err := svc.Create(tenantCtx(1), memberOfTenant(1),
		&model.CreateFormRequest{ApplicationID: 7, Name: "分组表单", ParentEntryCode: " menu_group "})
	assert.NoError(t, err)
...
func TestSaveDraft(t *testing.T) {
	...
	created, _ := svc.Create(tenantCtx(1), memberOfTenant(1), &model.CreateFormRequest{ApplicationID: 7, Name: "报名表"})

	// 非法协议：携带 issues 数据负载
	_, err := svc.SaveDraft(tenantCtx(1), memberOfTenant(1), created.ID, &model.SaveDraftRequest{
	...
	// 合法保存：口令递增
	result, err := svc.SaveDraft(tenantCtx(1), memberOfTenant(1), created.ID, &model.SaveDraftRequest{
		DraftRevision: created.DraftRevision,
		Content:       validDraft(),
	})
	assert.NoError(t, err)
...
func TestPublishAndRuntimeAndSubmit(t *testing.T) {
	...
	created, _ := svc.Create(ctx, member, &model.CreateFormRequest{ApplicationID: 7, Name: "报名表"})
	// 白名单外控件（user）拒绝发布并给出 issues
	_, err := svc.SaveDraft(ctx, member, created.ID, &model.SaveDraftRequest{
		DraftRevision: 1,
	...
	assert.NoError(t, err)
	_, err = svc.Publish(ctx, member, created.ID, &model.PublishRequest{DraftRevision: 2})
	assert.ErrorIs(t, err, apperrors.ErrPublishUnsupportedField)

	// 换成基础字段后发布成功：双口令
	_, _ = svc.SaveDraft(ctx, member, created.ID, &model.SaveDraftRequest{DraftRevision: 2, Content: validDraft()})
	published, err := svc.Publish(ctx, member, created.ID, &model.PublishRequest{DraftRevision: 3})
	assert.NoError(t, err)
	assert.Equal(t, 1, published.PublishedVersion)
	assert.NotEmpty(t, published.SchemaRevision)

	// 快照不可变：再次发布产生新版本，旧快照原样
	_, _ = svc.SaveDraft(ctx, member, created.ID, &model.SaveDraftRequest{
		DraftRevision: 3,
	...
	published2, err := svc.Publish(ctx, member, created.ID, &model.PublishRequest{DraftRevision: 4})
	assert.NoError(t, err)
	assert.Equal(t, 2, published2.PublishedVersion)

	// bootstrap：最新版本；归属不符/未发布路径
	runtime, err := svc.GetRuntime(ctx, "app_x", created.ID)
	assert.NoError(t, err)
	...
	_, err = svc.GetRuntime(ctx, "app_archived", created.ID)
	...
	result, err := svc.SubmitRecord(ctx, member, &model.SubmitRecordRequest{
		FormID: created.ID, PublishedVersion: 1, SchemaRevision: published.SchemaRevision,
	...
	_, err = svc.SubmitRecord(ctx, member, &model.SubmitRecordRequest{
		FormID: created.ID, PublishedVersion: 1, SchemaRevision: "999",
		Values: map[string]model.JSONContent{"_widget_a": model.JSONContent(`"张三"`)},
	})
...
func TestGetFormAndDelete(t *testing.T) {
	...
	detail, err := svc.Get(ctx, member, created.ID)
	assert.NoError(t, err)
	assert.Equal(t, "报名表", detail.Name)

	updated, err := svc.Update(ctx, member, created.ID, &model.UpdateFormRequest{Name: strPtr("新名称")})
	assert.NoError(t, err)
	assert.Equal(t, "新名称", updated.Name)

	assert.NoError(t, svc.Delete(ctx, member, created.ID))
	_, err = svc.Get(ctx, member, created.ID)
	assert.ErrorIs(t, err, apperrors.ErrFormNotFound)
}
...
func TestFormLifecycleMaintainsMenuEntries(t *testing.T) {
	...
	created, err := svc.Create(ctx, member, &model.CreateFormRequest{ApplicationID: 7, Name: "报名表"})
	assert.NoError(t, err)
	assert.Len(t, menu.attached, 1)
	assert.Equal(t, uint(7), menu.attached[0].appID)
	assert.Equal(t, created.ID, menu.attached[0].formID)
	assert.Equal(t, "报名表", menu.attached[0].name)
	// 未指定分组：按根级挂载（空 parentEntryCode）
	assert.Empty(t, menu.attached[0].parent)

	_, err = svc.Update(ctx, member, created.ID, &model.UpdateFormRequest{Name: strPtr("新名称")})
	assert.NoError(t, err)
	...
	assert.Equal(t, created.ID, menu.detached[0].formID)
}
